...
def analyze_video_content(video_path: str) -> str:
    """
    Analyzes a video file to produce a comprehensive summary of its editing, 
    pacing, visual content, and extracted text.

    Args:
        video_path (str): The file path to the video.

    Returns:
        str: A formatted textual summary of what the video does.
    """
    ...
    class VideoEditAnalyzer:
        ...
        def detect_black_frames(self, threshold=15):
            black_frames = []
            ...
            while True:
                ...
                if np.mean(gray) < threshold:
                    ...
                    if consecutive_black >= int(self.fps * 0.5):
                        ...
                        if consecutive_black > self.fps * 1.5: b_type = 'Fade to black'
                        elif consecutive_black > self.fps * 0.8: b_type = 'Medium black'
                        else: b_type = 'Quick black'
                        
                        black_frames.append({
                            'start_time': black_start, 
                            'duration': duration, 
                            'type': b_type
                        })
                    consecutive_black = 0
        ...
        def detect_transitions(self):
            if 'scenes' not in self.results: return
            scenes = self.results['scenes']
            transitions = []
            
            for i in range(len(scenes) - 1):
                gap = scenes[i+1]['start_time'] - scenes[i]['end_time']
                if gap < 0.05: t_type = 'Hard Cut'
                elif gap < 0.2: t_type = 'Quick Fade'
                elif gap < 0.8: t_type = 'Standard Dissolve'
                elif gap < 2.0: t_type = 'Long Fade'
                else: t_type = 'Pause/Gap'
                
                transitions.append({'type': t_type, 'gap': gap})
            
            self.results['transitions'] = transitions

        def extract_and_analyze_keyframes(self, num_frames=12):
            keyframes = []
            ...
            for i in range(0, self.total_frames, intervals):
                ...
                for bbox, text, conf in easy_res:
                    if conf > 0.3: # Filter low confidence
                        ...
                        if avg_y < h/3: pos = "Top"
                        elif avg_y < 2*h/3: pos = "Middle"
                        else: pos = "Bottom"
    ...
    try:
        ...
        summary.append("="*40)
        summary.append(f"File: {analyzer.video_name}")
        summary.append(f"Duration: {timedelta(seconds=analyzer.duration)}")
        summary.append(f"FPS: {analyzer.fps:.2f}")
        
        # Scene & Pacing
        if 'pacing' in res:
            p = res['pacing']
            summary.append(f"\nPACING: {p['pacing_category']}")
            summary.append(f"   • Total Shots: {p['total_shots']}")
            summary.append(f"   • Avg Shot Duration: {p['avg_shot_duration']:.2f}s")
            summary.append(f"   • Cuts per Minute: {p['shots_per_minute']:.1f}")
        
        # Transitions
        if 'transitions' in res:
            t_counts = defaultdict(int)
            for t in res['transitions']: t_counts[t['type']] += 1
            most_common = max(t_counts, key=t_counts.get) if t_counts else "None"
            summary.append(f"\nEDITING STYLE:")
            summary.append(f"   • Dominant Transition: {most_common}")
            summary.append(f"   • Transition Counts: {dict(t_counts)}")
            
        # Black Frames
        if 'black_frames' in res and res['black_frames']:
            summary.append(f"\nBLACK SEQUENCES: {len(res['black_frames'])} detected")
            for bf in res['black_frames'][:3]:
                summary.append(f"   • At {bf['start_time']:.1f}s ({bf['duration']:.1f}s): {bf['type']}")
        ...
        if 'keyframes' in res:
            for kf in res['keyframes']:
                if kf['detected_text'] and kf['detected_text'] != "No text":
                    summary.append(f"   • @ {kf['timestamp']:.1f}s: {kf['detected_text']}")
                    text_found = True
        
        if not text_found:
            summary.append("   • No significant on-screen text detected.")
